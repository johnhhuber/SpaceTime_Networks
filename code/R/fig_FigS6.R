...
scalars <- read.csv('../../data/eswatini_inference/time_travel/scalars_pooled.csv')
load('../../data/eswatini_inference/time_travel/networks_processed.RData')

# get posterior imported probability 
imported.prob <- network.data[[1]]$edgeProbs[1,-1]

# load median posterior estimates 
load('../../data/eswatini_inference/time_travel/validation/validation_parameters.RData')

# estimate median number of imported 
num.imported <- round(numNodes * median.founder.prob)

# plot 
tiff(filename = '../../output/figs/S6_Fig.tiff', width = 6.5, height = 5, units = 'in', res = 300, compression = 'lzw')

par(mfrow = c(2,1), mar = c(4.1, 4.1, 1.1, 1.1))
x <- seq(from = 0, to = 1, by = 0.001)

plot(x, dbeta(x, shape1 = tau.s.alpha.prior, shape2 = tau.s.beta.prior), type = 'n',
     bty = 'n', las = 1, xaxs = 'i', yaxs = 'i', xlab = '', ylab = '', ylim = c(0, 1.3 * max(dbeta(x, shape1 = tau.s.alpha.prior, shape2 = tau.s.beta.prior))),
     axes = F)
...
hist(scalars$tau_s, add = T, freq = F, col = col2alpha(palette[5], alpha = 0.75), border = 'white')
...
polygon(c(x, rev(x)), c(rep(0, length(x)), rev(dbeta(x, shape1 = tau.l.alpha.prior, shape2 = tau.l.beta.prior))),
        col = col2alpha('darkgrey'), border = NA)
hist(scalars$tau_l, add = T, freq = F, col = palette[5], border = 'white')
locals <- which(imported.prob < 0.25 & !is.na(nodes$History))
# calculate the posterior distribution for tau_s and tau_l 
tau.l.alpha.posterior <- tau.l.alpha.prior + sum(nodes$History[locals])
tau.l.beta.posterior <- tau.l.beta.prior + length(locals) - sum(nodes$History[locals])
...
legend('topleft', pch = c(15, 15, NA), lwd = c(NA, NA, 2), lty = c(NA, NA, 1), col = c('darkgrey', palette[5], '#222222'),
